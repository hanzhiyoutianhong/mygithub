...
public class LMApplicationDaoImpl implements LMApplicationDao {
    ...
    public static final String INSERT_APPLICATION = "INSERT INTO application(app_name, app_key_live, app_secret_live, app_key_test, app_secret_test, account_id, url_scheme, ios_store_url, ios_custom_url, ios_not_url, ios_bundle_id, ios_team_id, package_name, android_custom_url, android_not_url, desktop_url, timestamp)" + " VALUES(?,?,?,?,?,?,?,?,?,?,?,?)";
    ...
    public List<LMApplicationEntity> getAllApplication() {
        List<LMApplicationEntity> allRowsList = jdbcTemplate.query(QUERY_ALL_APPLICATION, new BeanPropertyRowMapper(LMApplicationEntity.class));
        return allRowsList;
    }

    public LMApplicationEntity getOneApplicationLive(String app_key_live) {
        Object[] values = {app_key_live};
        List<LMApplicationEntity> allRowList = jdbcTemplate.query(QUERY_ONE_APPLICATION_LIVE, values, new BeanPropertyRowMapper(LMApplicationEntity.class));
