...
public class DeviceDaoImpl extends BaseDao implements DeviceDao{
    ...
    public int addDevice(DeviceInfo deviceInfo) {
        int result = 0;
        try {
            TableChannel tableChannel = tableContainer.getTableChannel("device", ADD_DEVICE,  0L, 0L);

            Object[] values = {deviceInfo.getAppId(), deviceInfo.getDeviceId(),deviceInfo.getDeviceName(),
                deviceInfo.getPlatform(),deviceInfo.getDescription()};

            result += tableChannel.getJdbcTemplate().update(tableChannel.getSql(), values);
        } catch (DataAccessException e) {
            if (DaoUtil.isDuplicateInsert(e)) {
                ApiLogger.warn(new StringBuilder(128).append("Duplicate insert device table, deviceId=").append(deviceInfo.getDeviceId()), e);
                throw new LMException(LMExceptionFactor.LM_FAILURE_DB_OP, "duplicate insert device table, deviceId=" + deviceInfo.getDeviceId());
            }
    ...
    public int delDevice(long appId, String deviceId) {
        ...
        TableChannel tableChannel =
                tableContainer.getTableChannel("device", DEL_DEVICE_BY_APPID_AND_DEVICEID, 0L, 0L);
    ...
    public int updateDevice(DeviceInfo deviceInfo) {
        ...
        Object[] values = new Object[] {deviceInfo.getDeviceName(),deviceInfo.getPlatform(),deviceInfo.getDescription(),
                updateTime,deviceInfo.getAppId(),deviceInfo.getDeviceId()};
