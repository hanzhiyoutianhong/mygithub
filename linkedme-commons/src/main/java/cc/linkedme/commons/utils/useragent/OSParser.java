...
public class OSParser {
    ...
    protected static OSPattern patternFromMap(Map<String, String> configMap) {
        ...
        return (new OSPattern(Pattern.compile(regex), configMap.get("os_replacement"), configMap.get("os_v1_replacement"),
    ...
    protected static class OSPattern {
        ...
        public OS match(String agentString) {
            ...
            if (osReplacement != null) {
                if (groupCount >= 1) {
                    family = Pattern.compile("(" + Pattern.quote("$1") + ")").matcher(osReplacement).replaceAll(matcher.group(1));
                } else {
